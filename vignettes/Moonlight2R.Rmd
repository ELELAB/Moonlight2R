...
The user can perform text mining on candidate driver genes predicted from `DMA` 
using the `GTM` function. This function takes as input candidate driver genes 
and a query and finds associated PubMed IDs. Standard PubMed syntax can be used 
in the query. For example, Boolean operators AND, OR, NOT can be applied and 
tags such as [AU], [TITLE/ABSTRACT], [Affiliation] can be used. 
