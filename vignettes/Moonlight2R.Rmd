...
the network using the parmigene package. For illustrative purposes and to increase runtime, we 
have set `nGenesPerm = 5` and `nBoot = 5` in the below example, however, recommend setting 
these parameters to `nGenesPerm = 2000` and `nBoot = 400` to achieve optimal results.

```{r, eval = TRUE}
data(DEGsmatrix)
data(dataFilt)
dataGRN <- GRN(TFs = sample(rownames(DEGsmatrix), 100), normCounts = dataFilt,
	               nGenesPerm = 5, kNearest = 3, nBoot = 5)
```
