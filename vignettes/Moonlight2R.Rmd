...
- id: ref26
  ...
  author: 
  - family:  Colaprico, Antonio and Olsen, Catharina and Bailey, Matthew H. and Odom, Gabriel J. and Terkelsen, Thilde and Silva, Tiago C. and Olsen, André V. and Cantini, Laura and Zinovyev, Andrei and Barillot, Emmanuel and Noushmehr, Houtan and Bertoli, Gloria and Castiglioni, Isabella and Cava, Claudia and Bontempi, Gianluca and Chen, Xi Steven and Papaleo, Elena
  given:
...
To run the vignette, you also need the following packages: `tidyverse`, `EDASeq`,
`TCGAbiolinks` and `edgeR` which can be installed in the following way.
```{r, eval = FALSE}
...
using the R package TCGAbiolinks. Below is an example of downloading gene 
expression data, mutation data and obtaining differentially expressed genes 
from the TCGA-LUAD project using TCGAbiolinks.


## `Obtain gene expression data`: Get gene expression data matrix  [Gene Expression]

Below is an example of how to download and process gene expression data from 
the TCGA-LUAD project using TCGAbiolinks.
...
query_prepare <- GDCprepare(query = query_exp,
                            save = TRUE,
                            directory =  "GDCdata",
                            save.filename = "exp_data.rda")

## Process data
dataPrep <- TCGAanalyze_Preprocessing(object = query_prepare,
                                      cor.cut = 0.6)
...
## `Obtain mutation data`: Get MAF data [Mutation]

Below is an example of how to download mutation data in the form of a MAF file 
of the TCGA-LUAD project using TCGAbiolinks. 3000 mutations are sampled to 
...
Differential expression analysis is able to identify genes or probes that are significantly different 
between two phenotypes such as normal vs. tumor, or normal vs. stageI, normal vs. molecular subtype.
Below is an example of how to obtain differentially expressed genes between LUAD cancer samples and normal 
samples from the TCGA-LUAD project using TCGAbiolinks.
...
DEGsmatrix <- TCGAanalyze_DEA(mat1 = dataFilt[,dataSmNT],
...
We can visualize those DEGs with a volcano plot using the `TCGAVisualize_volcano` function from `TCGAbiolinks`.
...
### Introduction 
This vignette shows a complete workflow of the 'Moonlight2R' package except for the data download.
The code is divided into four case studies:

* 1. Downstream analysis LUAD 
* 2. Expression pipeline with Moonlight function
* 3. Expression pipeline with stages I II III IV (BRCA)
* 4. Moonlight with driver mutation analysis
...
## Case study n. 2: Expression pipeline with Moonlight function 
...
# Process TCGA-BRCA gene expression data
dataPrep <- TCGAanalyze_Preprocessing(object = query_prepare,
                                      cor.cut = 0.6)
...
# Get tumor stage information of samples
stage <- 1:4
curStage <- paste0("Stage ", as.roman(stage))
dataClin$tumor_stage <- toupper(dataClin$ajcc_pathologic_stage)
dataClin$tumor_stage <- gsub("[ABCDEFGH]","",dataClin$tumor_stage)
dataClin$tumor_stage <- gsub("ST","Stage",dataClin$tumor_stage)

# subset dataClin to only contain samples with stage I,II,III,IV info
dataStg <- dataClin[dataClin$tumor_stage %in% curStage,]

# get samples of stage I
dataStg_I <- dataStg[dataStg$tumor_stage == "Stage I",]
dataSmTP_I <- dataSmTP[substr(dataSmTP,1,12) %in% dataStg_I$bcr_patient_barcode]

# get samples of stage II
dataStg_II <- dataStg[dataStg$tumor_stage == "Stage II",]
dataSmTP_II <- dataSmTP[substr(dataSmTP,1,12) %in% dataStg_II$bcr_patient_barcode]

# get samples of stage III
dataStg_III <- dataStg[dataStg$tumor_stage == "Stage III",]
dataSmTP_III <- dataSmTP[substr(dataSmTP,1,12) %in% dataStg_III$bcr_patient_barcode]

# get samples of stage IV
dataStg_IV <- dataStg[dataStg$tumor_stage == "Stage IV",]
dataSmTP_IV <- dataSmTP[substr(dataSmTP,1,12) %in% dataStg_IV$bcr_patient_barcode]

listSmTP_stages <- list(dataSmTP_I, dataSmTP_II, dataSmTP_III, dataSmTP_IV)

# Find DEGs between each stage and normal samples and run Moonlight on each 
# set of DEGs
listMoonlight <- NULL
for (i in 1:4){
    DEGsmatrix <- TCGAanalyze_DEA(mat1 = dataFilt[,dataSmNT],
                                mat2 = dataFilt[,listSmTP_stages[[i]]],
                                Cond1type = "Normal",
                                Cond2type = "Tumor",
                                fdr.cut = 0.05,
                                logFC.cut = 1,
                                pipeline = "limma",
                                voom = TRUE)
    dataDual <- moonlight(dataDEGs = DEGsmatrix,
                        dataFilt = dataFilt[,listSmTP_stages[[i]]],
                        nTF = 5,
                        DiffGenes = TRUE,
                        BPname = c("apoptosis","proliferation of cells"),
                        dataMAF = dataMAF,
...
dataSmTP <- dataMAF$Tumor_Sample_Barcode
                
                dataStgC <- dataSmTP[substr(dataSmTP,1,12) %in% dataStg$bcr_patient_barcode]
                dataSmTP <- dataStgC
  
                info.mutation <- dataMAF[dataMAF$Tumor_Sample_Barcode %in% dataSmTP,]
...
## Case study n. 4: Moonlight with driver mutation analysis  
...
# Download TCGA-LUAD gene expression data
query_exp <- GDCquery(project = "TCGA-LUAD",
                      data.category = "Transcriptome Profiling",
                      data.type = "Gene Expression Quantification",
                      workflow.type = "STAR - Counts",
...
# Download TCGA-LUAD mutation data
...
# Find differentially expressed genes between TCGA-LUAD and normal samples
# Get tumor samples
dataSmTP <- TCGAquery_SampleTypes(barcode = colnames(dataFilt), typesample = "TP")
