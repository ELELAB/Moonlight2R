...
vignette: >
  %\VignetteIndexEntry{Moonlight2: An automatized workflow to study mechanistic indicators for driver gene prediction}
...
## `GTM`: Gene Text Mining
The user can perform text mining on driver genes predicted from `DMA` 
using the `GTM` function. This function takes as input driver genes, 
...
[TITLE/ABSTRACT], [Affiliation] can be used. `GTM` fetches data of 
PubMed records matching the specified query and extracts sentences 
from the abstract that contain either of the words supplied in the 
query. This is done for each of the genes supplied in the input. These 
sentences along with the PubMed ID, doi, title, full abstract, year of 
publication, and total number of PubMed records are returned in a table. 
