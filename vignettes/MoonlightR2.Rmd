...
We propose `MoonlightR2` a new approach to define TSGs and OCGs based on functional enrichment analysis, infer gene regulatory networks and upstream regulator analysis to score the importance of well-known biological processes with respect to the studied cancer.
...
<sup>1</sup> For the devel version of MoonlightR2 we use a short extract of ten biological functions from QIAGEN'S Ingenuity Pathway Analysis (IPA). We are still working to integrate the\Biocpkg{ReactomePA} package.
...
plotDMA(DEG_Mutations_Annotations, 
        ...
        gene_type = 'complete', n = 50, additionalFilename = "./DMAresults/")
```
...
### Introduction 
This vignette shows a complete workflow of the 'MoonlightR2' package except for the data download.
