...
genes <- append(dataDMA$TSG, dataDMA$OCG)

plotDMA(DEG_Mutations_Annotations, 
        Oncogenic_mediators_mutation_summary, 
        genelist = genes, additionalFilename = "./DMAresults/")
```


```{r, eval=FALSE, warning = FALSE, message = FALSE, include=FALSE}
pdftools::pdf_convert("heatmap_genelist.pdf", filename = "heatmap_genelist.png", format = "png", dpi = 300)
```

```{r, fig.width=6, fig.height=4, echo = FALSE, fig.align="center",hide=TRUE, message=FALSE,warning=FALSE}
img <- readPNG("heatmap_genelist.png")
grid.raster(img)
```

In the following plot all oncogenic mediators are shown with their mutations. 
...
```{r, eval=FALSE, warning = FALSE, message = FALSE, include=FALSE}
pdftools::pdf_convert("./heatmap_complete.pdf", filename= "heatmap_complete.png", format = "png", dpi = 300)
```
...
## OBS! This function is not yet inclueded. 
...
The figure generated by the code above is shown below:
```{r, eval=FALSE, warning = FALSE, message = FALSE, include=FALSE}
pdftools::pdf_convert("./moonlight_heatmap.pdf", filename = "moonlight_heatmap.png", format = "png", dpi = 300)
```
