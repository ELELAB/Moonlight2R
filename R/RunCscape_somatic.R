...
RunCscape_somatic <- function(input,
  ...
  cscape_annot <- cscape_in %>% 
    mutate(file_coding = coding_file, file_noncoding = noncoding_file) %>%
...
utils::globalVariables(c("Ranges", "Start_Position", "Reference_Allele", "Mutant", 
                         "file_coding", "file_noncoding", "Mydata", "Remark"))
