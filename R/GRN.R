...
GRN <- function(TFs, 
  ...
  if (DiffGenes == TRUE) {
    ...
    commonGenes <- intersect(rownames(DEGsmatrix), 
                             rownames(normCountsB))
  ...
  MRcandidates <- intersect(rownames(normCountsA),
                            TFs)
  ...
  tfListCancer <- TFs
  tfListCancer <- intersect(tfListCancer,
                            rownames(normCountsA))
  
  maxmi <- rep(0,
               length(tfListCancer))
  
  Cancer_null_distr <- matrix(0,
                              length(tfListCancer),
                              nBoot)
  ...
  for (i in seq.int(nBoot)) {
    ...
    g <- sample(seq.int(nrow(normCountsA)), 
                nGenesPerm)
  ...
  return(list(miTFGenes = miTFGenes, 
              maxmi = maxmi))
