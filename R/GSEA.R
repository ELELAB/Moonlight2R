...
GSEA <- function(DEGsmatrix, 
  ...
  dataDEGsnew <- cbind(mRNA = rownames(DEGsmatrix), 
                       DEGsmatrix)
  ...
  dataDEGsFiltLevel <- dataDEGsFiltLevel[order(dataDEGsFiltLevel$mRNA,
                                               decreasing=FALSE), ]
  ...
  dataDEGsFiltLevel_sub <- subset(dataDEGsFiltLevel, 
                                  select = c("GeneID", 
                                             "logFC"))
  ...
  genelistDEGs_sort <- sort(genelistDEGs,
                            decreasing = TRUE)
