...
FEA <- function(BPname = NULL, 
  ...
  for (k in seq.int(lf2)) {
    ...
    res <- as.data.frame(matrix(0,
                                ...
                                ncol = 7, 
                                dimnames = list(1,
                                                c("Diseases.or.Functions.Annotation",
    ...
    res$commonNg <- length(intersect(GeneList$PROBE_ID, 
    ...
    allgene <- unique(c(as.character(unique(EAGenes[, 
                                                    "ID"])), 
                        GeneList$PROBE_ID, 
                        selected_diseases$ID))
    ...
    selected_diseases <- selected_diseases[selected_diseases$ID %in% GeneList[,
                                                                              "PROBE_ID"], ]
    selected_diseases[, 
                      "Exp.Log.Ratio"] <- gsub(",", 
                                               ".",
                                               selected_diseases[, 
                                                                 "Exp.Log.Ratio"])
    selected_diseases[, 
                      "Exp.Log.Ratio"] <- as.numeric(selected_diseases[, 
                                                                       "Exp.Log.Ratio"])
    
    rownames(selected_diseases) <- selected_diseases$ID
    
    res$Molecules <- paste0(GeneList$PROBE_ID, 
                            collapse = ",")
    ...
    for (idx in seq.int(nrow(selected_diseases))) {
      ...
      if (length(grep("Increases",
                      selected_diseases[currTR,
                                        "Findings"])) == 1) {

        if (sign(GeneList[currTR,
                          "logFC"]) > 0) {

          selected_diseases[currTR,
                            "Prediction..based.on.expression.direction."] <- "Increased"

        }
        else if (sign(GeneList[currTR,"logFC"]) < 0) {

          selected_diseases[currTR,
                            "Prediction..based.on.expression.direction."] <- "Decreased"

        }
      }
      
      if (length(grep("Decreases",
                      selected_diseases[currTR,"Findings"])) == 1) {
    ...
    Correlation <- matrix(0,
                          nrow(selected_diseases),
                          1)

    selected_diseases <- cbind(selected_diseases, 
                               Correlation)
    
    if (length(grep("Decreases",
                    selected_diseases$Findings)) != 0) {
      
      selected_diseases[grep("Decreases",
                             selected_diseases$Findings),
                        "Findings"] <- -1
      selected_diseases[grep("Increases",
                             selected_diseases$Findings),
                        "Findings"] <- 1
      selected_diseases[grep("Affects",
                             selected_diseases$Findings),
                        "Findings"] <- 0
      selected_diseases[,
                        "Findings"] <- as.numeric(selected_diseases[,
                                                                    "Findings"])
      ...
      selected_diseases[PredictionIncreased,
                        "Correlation"] <- 1
      selected_diseases[PredictionDecreased,
                        "Correlation"] <- -1
      selected_diseases[PredictionAffected,
                        "Correlation"] <- 0
    ...
    TableDiseasesNew <- rbind(TableDiseasesNew, 
                              res)
  }
  
  close(pb)
  
  TableDiseasesNew <- cbind(TableDiseasesNew, 
                            FDR = p.adjust(TableDiseasesNew$p.Value,
                                           method = "fdr"))
