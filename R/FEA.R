...
FEA <- function(BPname = NULL,
  ...
  if (method == "fgsea") {
    ...
    pvals <- pmax(DEGsmatrix$PVal, .Machine$double.xmin)
    rankings <- sign(DEGsmatrix$logFC)*(-log10(pvals))
    names(rankings)  <- rownames(DEGsmatrix)
    ...
    fgseaRes <- fgsea(pathways = pathwayNamesList, stats = rankings, scoreType = 'std', minSize = 10, maxSize = max_pathway_size, nproc = 1)
    ...
    TableDiseasesNew <- TableDiseasesNew %>% rename("Diseases.or.Functions.Annotation" = pathway,
                                  "p.value" = pval)
