...
FEA <- function(BPname = NULL,
  ...
  for (k in seq.int(lf2)) {
    ...
    if (length(grep("Decreases", selected_diseases$Findings)) != 0) {
      ...
      selected_diseases[grep("Decreases", selected_diseases$Findings),
                        "Findings"] <- -1

      selected_diseases[grep("Increases", selected_diseases$Findings),
                        "Findings"] <- 1

      selected_diseases[grep("Affects", selected_diseases$Findings),
                        "Findings"] <- 0

      selected_diseases[,
                        "Findings"] <- as.numeric(selected_diseases[, "Findings"])
