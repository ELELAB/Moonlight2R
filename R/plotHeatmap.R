...
plotHeatmap <- function(df) {
  ...
  driver_mut_heatmap <- df %>% 
    ...
    heatmap(.row = Mutation_type,
            ...
            column_title = paste("Heatmap Driver annotation by CScape-Somatic, from gene:", first_gene,
                                 "\n Hugo_Symbol")) %>% 
...
utils::globalVariables(c("Mutation_type", "Hugo_Symbol", "Count", "Total_Mutations", 
                         "logFC", "Oncogenic_mediators_mutation_summary"))
